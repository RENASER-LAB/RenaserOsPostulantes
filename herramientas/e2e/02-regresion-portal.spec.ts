...
test.describe('Regresión · crear cuenta, que ahora exige ciudad', () => {
  ...
  test('sin ciudad NO deja pasar: sale el error y el campo queda inválido', async ({ page }) => {
    ...
    await page.getByLabel(/^Apellidos/).fill('QA Sin Ciudad')
    await page.getByLabel(/^Correo/).fill(`qa.sinciudad.${Date.now()}@ejemplo.pe`)
    ...
    await expect(page.getByLabel('Ciudad')).toHaveAttribute('aria-invalid', 'true')
  ...
  test('el alta completa con ciudad funciona de principio a fin', async ({ page }) => {
    ...
    await page.getByLabel(/^Apellidos/).fill('Quality Assurance')
  ...
  test('«Fuera del Perú» (EXT) también se puede elegir y el backend lo acepta', async ({ page }) => {
    ...
    await page.goto('/registro')
    await page.getByLabel(/^Nombre/).fill('Ext')
    await page.getByLabel(/^Apellidos/).fill('De Fuera')
    await page.getByLabel(/^Correo/).fill(correo)
    await page.getByLabel(/^Contraseña/).fill('Demo12345!')
    await page.getByLabel(/Repetir|Repite/i).fill('Demo12345!')
    await page.getByLabel('Ciudad').selectOption('EXT')
    await page.locator('input[type="checkbox"]').first().check()
    await page.getByRole('button', { name: /Crear cuenta/i }).click()
    await expect(page).not.toHaveURL(/\/registro/, { timeout: 15_000 })
  })
