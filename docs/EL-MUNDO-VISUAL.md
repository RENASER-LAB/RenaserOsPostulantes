...
| `src/app/Armazon.tsx` | La cabecera en píldora, el pie en columnas, y **el cielo de la portada**, que vive aquí porque tiene que pasar por detrás de la cabecera. También decide qué pantallas se centran en la ventana (`/ingresar`, `/admin/entrar`) y cómo sube «Inicio» |
| `src/paginas/vacantes/` | La portada —cielo, ventana de cristal, cinta, vacantes, «Por qué este proceso es distinto», preguntas— y la ficha de vacante. **Públicas** |
