...
| Pruebas unitarias (`npm test`) | 1 026 | **1 054** (+28) |
| Archivos con `serial` | 15 | 14 (se quitó el de `18-ranking-contra-api`) |
| Pruebas que fallaban en cada corrida | 16 (+8 más de `03-orden`, que no estaba en esa corrida) | 0 conocidas: las 24 dependían de un escenario que la siembra no produce, y ahora se intercepta |
...
| Tiempo de la suite | ~12 min | **por medir** en la segunda pasada, con el preview arriba (ver «Qué falta medir») |
...
2. **`sembrarEscenarioEnBase()`** y el guion `herramientas/e2e/sembrar-escenario-desarrollador-web.ts`:
   lo mismo escrito en la base por SQL, con `insert … on conflict` para la nota. Es la copia en
   TypeScript del script de Python, para medir una vía contra la otra.

Con `E2E_ESCENARIO=base` la intercepción se apaga y las pruebas se fían de lo que haya en la
base. El script de Python quedó corregido (cuenta las filas que escribe y para si no es una;
admite `--usuario`, porque el clon del harness no tiene rol `postgres`) y **no se borra hasta
que se midan los tiempos**.
...
| cambiar de pestaña con filtros puestos los limpia, pero el corte se conserva | R | absorbe la siguiente: ahora también mira el orden |
...
Las tres son R: siembra y restaura su propio caso contra la base. Es el precedente de la
siembra propia en TypeScript.

### 07-movil (10 → 9, proyecto `movil`)

| Prueba | Cubo | Nota |
|---|---|---|
| la tabla no desborda el body | R | |
...
| recorrer el ranking no levanta errores nuevos en consola | R | |

### 09-avance (1 → 1) · 10-panel-entrar (6 → 6) · 11-perfil (13 → 13) · 12-postular (4 → 4)

Todas R. `10`, `11` y `12` siguen en `serial` (ver abajo).
...
| la misma ficha en «Prueba del puesto» enseña su rúbrica, no el CV | R | |
...
Todas R, en `serial` (cada tramo abre lo que dejó el anterior).
...
| el escape a la tanda entera trae a todos, y cada guion dice por qué | R | |
...
| Prueba | Cubo | Nota |
|---|---|---|
| el lápiz abre un modal con los datos de ahora, y la tabla no se mueve | R | |
...
## Las que dependían del sembrador, y qué falta medir (AC-10)

Las pruebas que dependían de `scripts/sembrar-escenario-e2e.py` y siguen como e2e:

| Prueba | Vía por defecto ahora | Comando para medirla |
|---|---|---|
...
| `08-teclado` «la barra de filtros entera se recorre con Tab» | interceptada | `npx playwright test herramientas/e2e/08-teclado-y-consola.spec.ts -g "Tab"` |

Las otras diecinueve que dependían de él bajaron a unitario o sobraban (tabla de arriba); la
de `20` fallaba por formato.

**Cómo medir cada lado**, con el preview arriba y las seis variables de
[TRABAJAR-EN-LOCAL.md](TRABAJAR-EN-LOCAL.md) exportadas, sobre el mismo clon:

```bash
# A · el sembrador de Python + las pruebas fiándose de la base
...
El tiempo de A y de B es la suma de sembrar y correr; el de C es solo correr. Los dos
sembradores dejan el escenario puesto en la base (como siempre hizo el de Python); en un clon
del harness eso no importa, y en una base propia es el escenario que las pruebas esperan.

| Prueba | A · Python | B · TypeScript | C · interceptar | Decisión |
|---|---|---|---|---|
| `03-orden` | _por medir_ | _por medir_ | _por medir_ | |
| `04-filtros` (rangos) | _por medir_ | _por medir_ | _por medir_ | |
| `05-excel` (tres filas) | _por medir_ | _por medir_ | _por medir_ | |
| `07-movil` (ordenar y filtrar) | _por medir_ | _por medir_ | _por medir_ | |
| `08-teclado` (Tab) | _por medir_ | _por medir_ | _por medir_ | |

Regla de la decisión: la prueba migra a la intercepción si es claramente más rápida; si el
sembrador es igual o más rápido, se queda con él (`E2E_ESCENARIO=base` pasaría a ser el modo
por defecto de esa prueba). **El script de Python se borra solo si, tras esta tabla, ninguna
prueba lo sigue necesitando.** Mientras tanto quedó corregido: cuenta las filas que escribe y
para si no es exactamente una, escribe la nota con `insert … on conflict`, y admite
`--usuario` (el clon del harness no tiene rol `postgres`).
...
- **Medir los tiempos** de la tabla de AC-10 y decidir el destino del script de Python (segunda
  pasada, con el preview arriba).
- **Correr la suite entera** en headless con el montaje manual y anotar aquí el tiempo total
  frente a los ~12 minutos de las 329 (AC-07). En esta pasada la aplicación no estaba arrancada.
...
E2E_ESCENARIO=base npx playwright test 03- 04- 05- 07- 08-   # fiándose de la base (medición)
```
